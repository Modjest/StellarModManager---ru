...
{
    [ObservableProperty]
    private string gamePath = "Игра не выбрана";
    ...
    public MainWindowViewModel()
    {
        ...
        if (profile != null)
        {
            ...
            {
                MelonLoaderStatusText = "Путь к ранее сохраненной игре больше недействителен.";
            }
        }
    }
    ...
    private bool CheckMelonLoader(string path)
    {
        ...
        {
            MelonLoaderStatus.Installed => "MelonLoader установлен",
            MelonLoaderStatus.NotInstalled => "MelonLoader не установлен",
            _ => "Невозможно прочиать эту папку"
        };
    ...
    private bool CanInstallMelonLoader() => GamePath != "Игра не выбрана" && !IsMelonLoaderValid && !IsInstallingMelonLoader;

    [RelayCommand(CanExecute = nameof(CanInstallMelonLoader))]
    private async Task InstallMelonLoader()
    {
        IsInstallingMelonLoader = true;
        InstallProgressText = "Запуск...";
        ...
        {
            MelonLoaderStatusText = $"Авто-установка провалилась: {ex.Message}"; // Hopesfully this never happens... ijdsfjipfdsipjhfdfdf
        }
        finally
        {
    ...
    {
        if (GamePath == "Игра не выбрана")
        {
            MelonLoaderStatusText = "Сначала выберите игру";
            return;
        }
        ...
        try
        {
            ...
            MelonLoaderStatusText = $"{mod.Name} скопирован в игру";
        }
        catch (Exception ex)
        {
            MelonLoaderStatusText = $"Деплой провален: {ex.Message}";
        }
        finally
        {
    ...
    private async Task RemoveMod(InstalledModInfo mod)
    {
        ...
        try
        {
            ...
            MelonLoaderStatusText = $"{mod.Name} удален";
        }
        catch (Exception ex)
        {
            MelonLoaderStatusText = $"Удаление провалено: {ex.Message}";
        }
        finally
        {
    ...
    private async Task UpdateMod(InstalledModInfo mod)
    {
        ...
        if (onlineMatch == null)
        {
            MelonLoaderStatusText = "Не могу найти этот мод в онлайн репозитории";
            return;
        }
        mod.IsUpdating = true;
        ...
        try
        {
            ...
            MelonLoaderStatusText = $"{mod.Name} обновлен до {onlineMatch.Version}. установи это в игру снова чтобы обновить";
        }
        catch (Exception ex)
        {
            MelonLoaderStatusText = $"Обновление провалено: {ex.Message}";
        }
        finally
        {
    ...
    private async Task InstallMod(OnlineModInfo mod)
    {
        ...
        try
        {
            ...
            MelonLoaderStatusText = $"{mod.Name} установлен";
        }
        catch (Exception ex)
        {
            MelonLoaderStatusText = $"Установка провалена: {ex.Message}";
        }
        finally
        {
